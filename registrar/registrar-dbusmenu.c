...
void registrar_dbus_menu_register_window (RegistrarDBusMenu* self, uint window_id, const char* menu_object_path, const char* sender) {
    ...
    g_signal_emit (self, registrar_dbus_menu_signals[WINDOW_REGISTERED_SIGNAL], 0, window_id, menu_object_path, sender);
}
...
void registrar_dbus_menu_get_menu_for_window(RegistrarDBusMenu* self, uint window_id, char** service, char** object_path)
{
    DBusAddress* addr = (DBusAddress*)g_hash_table_lookup(self->menus,GUINT_TO_POINTER(window_id));
    if(addr)
    {
...
static void registrar_dbus_menu_class_init (RegistrarDBusMenuClass * klass) {
    ...
    registrar_dbus_menu_signals[WINDOW_REGISTERED_SIGNAL] = g_signal_new ("window-registered", registrar_dbus_menu_get_type(), G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_user_marshal_VOID__UINT_STRING_STRING, G_TYPE_NONE, 3, G_TYPE_UINT, G_TYPE_STRING, G_TYPE_STRING);
    registrar_dbus_menu_signals[WINDOW_UNREGISTERED_SIGNAL] = g_signal_new ("window-unregistered", registrar_dbus_menu_get_type(), G_SIGNAL_RUN_LAST, 0, NULL, NULL, g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);
}

static void _dbus_registrar_dbus_menu_register_window (RegistrarDBusMenu* self, GVariant* _parameters_, GDBusMethodInvocation* invocation) {
    ...
    g_variant_unref (value);
    registrar_dbus_menu_register_window (self, window_id, menu_object_path, g_dbus_method_invocation_get_sender (invocation));
...
static void _dbus_registrar_dbus_menu_unregister_window (RegistrarDBusMenu* self, GVariant* _parameters_, GDBusMethodInvocation* invocation) {
    g_autoptr(GError) error = NULL;
    GVariantIter _arguments_iter;
    uint window_id = 0U;
    GDBusMessage* _reply_message = NULL;
...
static void _dbus_registrar_dbus_menu_get_menu_for_window (RegistrarDBusMenu* self, GVariant* _parameters_, GDBusMethodInvocation* invocation) {
    GError* error = NULL;
    GVariantIter _arguments_iter;
    uint window = 0U;
    GVariant* _tmp3_;
...
static void _dbus_registrar_dbus_menu_get_menus (RegistrarDBusMenu* self, GVariant* _parameters_, GDBusMethodInvocation* invocation) {
    GError* error = NULL;
    GVariantIter _arguments_iter;
    GDBusMessage* _reply_message = NULL;
    GVariant* _reply;
...
static void registrar_dbus_menu_dbus_interface_method_call (GDBusConnection* connection, const char* sender, const char* object_path, const char* interface_name, const char* method_name, GVariant* parameters, GDBusMethodInvocation* invocation, gpointer user_data) {
...
static GVariant* registrar_dbus_menu_dbus_interface_get_property (GDBusConnection* connection, const char* sender, const char* object_path, const char* interface_name, const char* property_name, GError** error, gpointer user_data)
{
    return NULL;
}


static bool registrar_dbus_menu_dbus_interface_set_property (GDBusConnection* connection, const char* sender, const char* object_path, const char* interface_name, const char* property_name, GVariant* value, GError** error, gpointer user_data)
{
    return false;
}


static void _dbus_registrar_dbus_menu_window_registered (GObject* _sender, uint window_id, const char* service, const char* path, gpointer* _data) {
    ...
    g_dbus_connection_emit_signal (_connection, NULL, DBUSMENU_REG_OBJECT, DBUSMENU_REG_IFACE, "WindowRegistered", _arguments, NULL);
}


static void _dbus_registrar_dbus_menu_window_unregistered (GObject* _sender, uint window_id, gpointer* _data) {
    GDBusConnection * _connection;
    const char * _path;
    GVariant *_arguments;
    GVariantBuilder _arguments_builder;
    ...
    g_variant_builder_add_value (&_arguments_builder, g_variant_new_uint32 (window_id));
    _arguments = g_variant_builder_end (&_arguments_builder);
    g_dbus_connection_emit_signal (_connection, NULL, DBUSMENU_REG_OBJECT, DBUSMENU_REG_IFACE, "WindowUnregistered", _arguments, NULL);
}
static const GDBusInterfaceVTable _interface_vtable = {registrar_dbus_menu_dbus_interface_method_call,
...
static void _registrar_dbus_menu_unregister_object (gpointer user_data) {
    ...
    g_signal_handlers_disconnect_by_func (data, _dbus_registrar_dbus_menu_window_registered, data);
    g_signal_handlers_disconnect_by_func (data, _dbus_registrar_dbus_menu_window_unregistered, data);
    g_object_unref (data);
}

uint registrar_dbus_menu_register (RegistrarDBusMenu* object, GDBusConnection* connection, GError** error) {
    uint result;
    gpointer *data;
    ...
    g_signal_connect (object, "window-registered", (GCallback) _dbus_registrar_dbus_menu_window_registered, connection);
    g_signal_connect (object, "window-unregistered", (GCallback) _dbus_registrar_dbus_menu_window_unregistered, connection);
    return result;
}
