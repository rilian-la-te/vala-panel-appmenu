...
static void
update_model (GtkTreeStore *tree_store,
              GtkTreeIter  *iter,
      ...
      if (!gtk_tree_model_get_iter_first (GTK_TREE_MODEL (tree_store), &parent_iter))
        {
          ...
          gtk_tree_store_set (tree_store, &parent_iter,
                              SHELL_COLUMN, menu_shell,
                              LABEL_COLUMN, "Menu bar",
                              -1);
        }
    }
  else
  ...
  while (menu_items != NULL)
    {
      ...
      gtk_tree_store_set (tree_store, &child_iter,
                          ...
                          SHELL_COLUMN, submenu,
                          LABEL_COLUMN, gtk_menu_item_get_label (menu_item),
                          -1);
...
static void
add_menu_bar (GtkToolButton *button,
              gpointer       user_data)
{
...
static void
remove_menu_bar (GtkToolButton *button,
                 gpointer       user_data)
{
...
static GtkMenuItem * get_menu_item (GtkMenuShell *menu_shell,
...
static GtkMenuShell *
get_menu_shell (GtkMenuShell *menu_shell,
                GtkTreeModel *tree_model,
                GtkTreeIter  *child_iter)
{
  if (child_iter != NULL)
    {
      ...
      if (gtk_tree_model_iter_parent (tree_model, &parent_iter, child_iter))
        return GTK_MENU_SHELL (gtk_menu_item_get_submenu (get_menu_item (menu_shell, tree_model, child_iter)));
      else
        return menu_shell;
    }
...
static GtkMenuItem *
get_menu_item (GtkMenuShell *menu_shell,
               GtkTreeModel *tree_model,
               GtkTreeIter  *child_iter)
{
  GtkTreeIter parent_iter;
...
static void
add_menu_item (GtkToolButton *button,
               gpointer       user_data)
{
  ...
  if (selection != NULL && gtk_tree_selection_get_selected (selection, NULL, &tree_iter))
    {
      ...
      submenu = GTK_WIDGET (get_menu_shell (GTK_MENU_SHELL (menu_bar), GTK_TREE_MODEL (tree_store), &tree_iter));

      if (submenu == NULL)
        {
          GtkMenuItem *menu_item;

          menu_item = get_menu_item (GTK_MENU_SHELL (menu_bar), GTK_TREE_MODEL (tree_store), &tree_iter);
      ...
      char *argv[])
{
  ...
  tree_view_column = gtk_tree_view_column_new_with_attributes ("Label", cell_renderer,
                                                               "text", LABEL_COLUMN,
                                                               NULL);
  ...
  cell_renderer = gtk_cell_renderer_text_new ();
  tree_view_column = gtk_tree_view_column_new_with_attributes ("Type", cell_renderer,
                                                               "text", TYPE_COLUMN,
                                                               NULL);
  ...
  cell_renderer = gtk_cell_renderer_text_new ();
  tree_view_column = gtk_tree_view_column_new_with_attributes ("Accel", cell_renderer,
                                                               "text", ACCEL_COLUMN,
                                                               NULL);
  gtk_tree_view_column_set_min_width (tree_view_column, 80);
  gtk_tree_view_append_column (GTK_TREE_VIEW (tree_view), tree_view_column);
  ...
  radio_button = gtk_radio_button_new_with_mnemonic_from_widget (GTK_RADIO_BUTTON (radio_button), "_Check");
  gtk_grid_attach (GTK_GRID (inner_grid), radio_button, 1, 2, 1, 1);

  radio_button = gtk_radio_button_new_with_mnemonic_from_widget (GTK_RADIO_BUTTON (radio_button), "_Radio");
  gtk_grid_attach (GTK_GRID (inner_grid), radio_button, 1, 3, 1, 1);

  radio_button = gtk_radio_button_new_with_mnemonic_from_widget (GTK_RADIO_BUTTON (radio_button), "_Image");
  gtk_grid_attach (GTK_GRID (inner_grid), radio_button, 1, 4, 1, 1);

  radio_button = gtk_radio_button_new_with_mnemonic_from_widget (GTK_RADIO_BUTTON (radio_button), "_Separator");
