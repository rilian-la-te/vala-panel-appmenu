...
static void
menubar_mode_changed (GSettings   *settings,
                      const gchar *key,
                      gpointer     user_data)
{
...
gboolean
menu_source_is_maximised (WnckWindowState state)
{
  WnckWindowState maximised_mask;

  maximised_mask = WNCK_WINDOW_STATE_MAXIMIZED_HORIZONTALLY | WNCK_WINDOW_STATE_MAXIMIZED_VERTICALLY;

  return (state & maximised_mask) == maximised_mask;
}

static void
menu_source_state_changed (WnckWindow      *window,
      ...
      for (i = 0; i < N_XPROP; i++)
        {
          ...
          if (menu_source_update_window_property (menu_source, i, property->state == PropertyDelete))
            menu_source_update (menu_source);
...
static void
menu_source_active_window_changed (WnckScreen *screen,
  ...
  if (active_window != menu_source->active_window)
    {
      ...
      if (menu_source->active_window != NULL)
        {
          gdk_window_remove_filter (menu_source->gdk_window, window_filter_func, menu_source);
          g_object_unref (menu_source->gdk_window);
          menu_source->gdk_window = NULL;

          g_signal_handler_disconnect (menu_source->active_window, menu_source->state_changed_id);
      ...
      if (active_window != NULL)
        {
          ...
          menu_source->state_changed_id = g_signal_connect (active_window, "state-changed",
                                                            G_CALLBACK (menu_source_state_changed), menu_source);
          menu_source->maximised = menu_source_is_maximised (wnck_window_get_state (active_window));

          xid = wnck_window_get_xid (active_window);
          menu_source->gdk_window = gdk_x11_window_foreign_new_for_display (gdk_display_get_default (), xid);
          gdk_window_add_filter (menu_source->gdk_window, window_filter_func, menu_source);
        }

      for (i = 0; i < N_XPROP; i++)
        changed |= menu_source_update_window_property (menu_source, i, active_window == NULL);
...
static void
menu_source_get_property (GObject *object, guint prop_id,
...
static void
menu_source_finalize (GObject *object)
{
  MenuSource *menu_source = MENU_SOURCE (object);
  ...
  if (menu_source->screen != NULL)
    {
      g_signal_handler_disconnect (menu_source->screen, menu_source->active_window_changed_id);
  ...
  G_OBJECT_CLASS (menu_source_parent_class)
    ->finalize (object);
}

static void
menu_source_init (MenuSource *menu_source)
{
}

static void
menu_source_class_init (MenuSourceClass *class)
{
  class->get_property = menu_source_get_property;
  class->finalize = menu_source_finalize;

  menu_source_menu_param = g_param_spec_object ("menu", "active menu", "the GMenuModel for the active window",
                                                G_TYPE_MENU_MODEL, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
  g_object_class_install_property (class, 1, menu_source_menu_param);
}

static MenuSource *
menu_source_new (GdkScreen *screen)
{
  ...
  g_signal_connect (menu_source->screen, "active-window-changed",
                    G_CALLBACK (menu_source_active_window_changed), menu_source);
...
GMenuModel *
menu_source_get_menu (MenuSource *menu_source)
{
...
MenuSource *
menu_source_get_for_screen (GdkScreen *screen)
{
  MenuSource *menu_source;
  GSList *node;
