...
static void layout_parse(DBusMenuModel *menu, GVariant *layout)
{
	...
	while ((child = g_variant_iter_next_value(&iter)))
	{
		...
		if (diff)
		{
			...
			{
				dbus_menu_item_generate_action(new_item, menu);
				g_print("Item %u updated by layout\n", new_item->id);
		...
		else
		{
			...
			g_print("Item %u updated by props\n", new_item->id);
	...
	section_num++;
	emit_layout_update_signal(menu, -1, 0, old_sections, section_num);
...
static void get_layout_cb(GObject *source_object, GAsyncResult *res, gpointer user_data)
{
	...
	dbus_menu_xml_call_get_layout_finish((DBusMenuXml *)(source_object),
	                                     ...
	                                     res,
	                                     &error);
	g_autofree char *props = g_variant_print(layout, true);
	g_print("%s\n", props);
	if (error != NULL)
	{
